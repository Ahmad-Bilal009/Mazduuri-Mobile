...
import { workersApi, uploadImage } from "@/lib/api";
...
export default function CreateWorkerScreen() {
  ...
  // Prefilled from the signup flow when available
  ...
  async function handleSubmit() {
    ...
    if (!dailyWage || Number(dailyWage) <= 0) {
      Alert.alert("Missing wage", "Please enter your daily wage in PKR.");
      return;
    }
    setSaving(true);
    try {
      await workersApi.upsertProfile({
        ...
        categoryIds,
        subcategoryIds,
        skills: mapCategoriesToSkills(categoryIds),
        availability: true,
      });
  ...
  const step2 = (
    ...
    >
      ...
      <Text style={styles.stepDesc}>
        Tell clients about your skills, rate, and experience.
      </Text>
